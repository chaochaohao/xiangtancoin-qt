...
//#1
unsigned int nStakeMinAge = 24 * 60 * 60 * 365; // 2 hours
unsigned int nStakeMaxAge = -1; // unlimited
unsigned int nModifierInterval = 50; // time to elapse before new modifier is computed
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    {
        nSubsidy = 0 * COIN; // 1 billion coins for ico
    }else if(pindexBest->nHeight <=100){
        nSubsidy = 500000 * COIN; // 1 billion coins for ico
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitBay", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //#1
        const char* pszTimestamp = "xiangtancoin gonna make an Impact on the altcoin market never seen before";
        CTransaction txNew;
        txNew.nTime = 1491206011;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
//        while (block.GetHash()> hashGenesisBlock)
//        {
//            block.nNonce = block.nNonce + 4;
//        }
        block.print();
        ...
        //#1
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x749fd011359850be4532e76a903e35f31a3bcf4da0788d5c3a5f99d055ffbb61"));
        block.print();
